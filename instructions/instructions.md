...
Setting up an FTCStats development environment takes a few steps. This guide will walk you through the process.

## Docker quickstart (easiest)

1. Install + start Docker Desktop.
2. From the `ftcstats/` directory:
   - `cp .env.example .env`
   - Edit `.env` and set `FTC_API_KEY_RAW="username:apikey"` (and optionally `SYNC_API=1`)
   - Run `npm run docker:up`
3. Open `http://localhost:3000`

## Setting up your database

FTCStats uses a Postgres SQL database. You must set it up separately before running FTCStats.
...
You must use an FTC Event Web API key to fetch the data used in FTCStats. You can get one by following the instructions [here](https://ftc-events.firstinspires.org/api).

Once you have been emailed your API key, it will be in the format XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX. This key needs to be reformatted for use with FTCStats. First, take the username you used when signing up (it is in the email) and put it to the left to the key, separated by a colon. Then, encode it in Base64.

`exampleusername:XXXX...XXXX` -> `ZXhhbXBsZXVzZX...`

This is the key you will use later when setting up your environment variables. Alternatively, you can skip the Base64 step and set `FTC_API_KEY_RAW="exampleusername:XXXX...XXXX"`; the server will Base64 it automatically.
...
You should now see your very own FTCStats instance running on your computer!
